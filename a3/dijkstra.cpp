...
void Graph::shortestPath(int src)
{
	...
	while (!setds.empty())
	{
		...
		for (i = adj[u].begin(); i != adj[u].end(); ++i)
		{
			...
			if (dist[v] > dist[u] + weight)
			{
				...
				//printf("u = %c and v = %c and their wight is %d\n",convertIntToABC(u),convertIntToABC(v),dist[v]);
	...
	for (char i = 'A'; i < ('A' + V); ++i)
	{
		int pos = i - 'A';
		char modSrc = src;
		char getSrc = convertIntToABC(src);
		//cout <<"Stuid crap "<< endl;
		//printf("%c \t\t %d\n", i, dist[(i-'A')]);
		printf("from %c -> %c the distance is %d\n", getSrc, i, dist[pos]);
	}
}
